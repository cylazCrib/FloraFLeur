        ...
        <main id="dashboard-view" class="view active-view">
            ...
            <section class="summary-cards">
                ...
                <div class="card card-inventory"><h2>Inventory</h2><p class="card-main-value">{{ $inventoryCount }} items</p>
                    <p class="card-sub-value" style="{{ $lowStockCount > 0 ? 'color:#D32F2F;font-weight:bold;' : '' }}">Low Stock: {{ $lowStockCount }}</p>
                </div>
            </section>
            ...
            <section class="content-container" style="margin-bottom: 2rem;">
                ...
                <div class="table-wrapper"><table class="data-table"><thead><tr><th>ID</th><th>Customer</th><th>Items</th><th>Total</th><th>Status</th><th>Action</th></tr></thead><tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $o->customer_name }}<br><small class="text-gray-400">{{ $o->customer_phone }}</small></td>
                        <td>{{ $o->items->map(fn($i)=>$i->quantity.'x '.$i->product_name)->join(', ') }}</td>
                        <td>₱{{ number_format($o->total_amount, 2) }}</td>
                        <td>
                            <select id="status-{{ $o->id }}" class="border p-1 rounded text-sm">
                        ...
                        <td class="flex gap-2">
                             ...
                             <button class="text-blue-600 hover:text-blue-800 view-order-btn" title="View" 
                                data-id="{{ $o->order_number }}" 
            ...
            <section class="content-container">
                ...
                <div class="table-wrapper"><table class="data-table"><thead><tr><th>Order #</th><th>Address</th><th>Driver</th><th>Action</th></tr></thead><tbody>
                     ...
                     <tr>
                        ...
                        <td>
                            <select id="driver-{{ $o->id }}" class="border p-1 rounded text-sm">
                        ...
                        <td class="flex gap-2">
                            ...
                            <a href="https://www.google.com/maps/search/?api=1&query=${{ urlencode($o->delivery_address) }}" target="_blank" class="text-gray-500 hover:text-red-500" title="Map"><i class="fa-solid fa-map-location-dot"></i></a>
                        </td>
                     </tr>
                    ...
                    <tr>
                        ...
                        <td class="flex gap-2">
                            <button class="text-blue-600 hover:text-blue-800 edit-product-btn" 
                                ...
                                data-price="{{ $p->price }}" 
                                data-cat="{{ $p->category }}"
                                data-occ="{{ $p->occasion }}"><i class="fa-regular fa-pen-to-square"></i></button>
                    ...
                    @forelse($items as $i)
                        <div class="inventory-item flex justify-between items-center p-4 bg-white border border-gray-100 rounded-lg shadow-sm mb-2">
                            ...
                            <div class="flex gap-2">
                                <button class="text-gray-400 hover:text-blue-600 update-item-btn" 
                                    data-id="{{ $i->id }}" data-name="{{ $i->name }}" data-code="{{ $i->code }}" data-quantity="{{ $i->quantity }}" data-type="item"><i class="fa-solid fa-pen"></i></button>
                                <button class="text-gray-400 hover:text-red-600 del-inv" data-id="{{ $i->id }}"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        </div>
                ...
                <div class="inventory-list">
                    @forelse($flowers as $item)
                        <div class="inventory-item flex justify-between items-center p-4 bg-white border border-gray-100 rounded-lg shadow-sm mb-2">
                            ...
                            <div class="flex gap-2">
                                <button class="text-gray-400 hover:text-blue-600 update-item-btn" 
                                    data-id="{{ $item->id }}" data-name="{{ $item->name }}" data-code="{{ $item->code }}" data-quantity="{{ $item->quantity }}" data-type="flower"><i class="fa-solid fa-pen"></i></button>
                                <button class="text-gray-400 hover:text-red-600 del-inv" data-id="{{ $item->id }}"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        </div>
        ...
        <main id="staff-view" class="view">
            ...
            <section class="content-container">
                ...
                <div class="table-wrapper"><table class="data-table"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Action</th></tr></thead><tbody>
                    @forelse($staff as $s)
                    <tr>
                        <td>{{ $s->name }}</td><td>{{ $s->email }}</td><td><span class="px-2 py-1 bg-gray-100 rounded text-xs font-bold">{{ $s->role }}</span></td>
                        <td><span class="status status-active">Active</span></td>
                        <td class="flex gap-2">
                            <button class="text-blue-600 hover:text-blue-800 edit-staff-btn" 
                                data-id="{{ $s->id }}" data-name="{{ $s->name }}" data-email="{{ $s->email }}" data-phone="{{ $s->phone }}" data-role="{{ $s->role }}"><i class="fa-regular fa-pen-to-square"></i></button>
